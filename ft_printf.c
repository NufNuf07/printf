...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2020/12/08 14:36:49 by visamiez          #+#    #+#             */
/*   Updated: 2020/12/08 14:36:52 by visamiez         ###   ########lyon.fr   */
/*                                                                            */
/* ************************************************************************** */
...
int			ft_save(const char *save, va_list args)
{
	...
	while (1)
	{
		...
		else if (save[i] == '%' && save[i + 1])
		{
			...
			if (ft_is_in_type_list(save[i]))
				char_count += ft_parse((char)flags.type, flags, args);
			else if (save[i])
